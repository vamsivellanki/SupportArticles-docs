...
#Customer intent: As an Azure Kubernetes user, I want to troubleshoot the VMExtensionError_CniDownloadTimeout error so that I can successfully create and deploy an Azure Kubernetes Service (AKS) cluster.
ms.custom: sap:Create, Upgrade, Scale and Delete operations (cluster or nodepool)
---
# Troubleshoot the VMExtensionError_CniDownloadTimeout error 

This article discusses how to identify and resolve the `VMExtensionError_CniDownloadTimeout` error (also known as error code `ERR_CNI_DOWNLOAD_TIMEOUT`,) that occurs when you try to create and deploy a Microsoft Azure Kubernetes Service (AKS) cluster.
